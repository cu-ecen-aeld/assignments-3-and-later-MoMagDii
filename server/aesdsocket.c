...
int main(int argc, char * argv[]){
    ...
    char buffer[5120];  //Total buffer size is 1kB
    ...
    while (1){
        ...
        while ((bytes_read = recv(client_fd, buffer, 5120, 0)) > 0) {   //Buffer set to 1kB
            if (bytes_read == -1){
        ...
        int bytes_send;
        while ((bytes_read = read(file_fd, &buffer, 5120)) > 0){    //Bytes and buffer set to 1024, 1kB
